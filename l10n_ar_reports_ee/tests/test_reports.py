...
class TestReports(TransactionCase):
    ...
    def _test_txt_file(self, filename):
        ...
        out_txt = self.vat_book._get_txt_files(self.options)[filetype].decode(
            "ISO-8859-1"
        )
        res_file = codecs.open(
            get_module_resource("l10n_ar_reports_ee", "tests", filename),
            "r",
            encoding="ISO-8859-1",
        ).read()
        ...
        res_file = res_file.replace(
            "20210331",
            (fields.Date.end_of(date_utils.add(today, months=1), "month")).strftime(
                "%Y%m%d"
            ),
        )
        ...
        self.assertEquals(
            out_txt,
            res_file,
            "The expected and the generated are different"
            + "\n\n ---- Current\n"
            + out_txt
            + "\n\n ---- Expected\n"
            + res_file,
