...
class AccountArVatLine(models.Model):
    ...
    icl_tax = fields.Monetary(readonly=True, string=_('ICL tax'), currency_field='company_currency_id')
    idc_tax = fields.Monetary(readonly=True, string=_('IDC tax'), currency_field='company_currency_id')
    net_total = fields.Monetary(readonly=True, string=_('Net total'), currency_field='company_currency_id')
    ...
    product_id = fields.Many2one('product.product', 'Product', readonly=True, auto_join=True)
...
SELECT
    ...
    CASE
        WHEN sum(ABS(CASE WHEN ntg.l10n_ar_vat_afip_code IS NULL AND ntg.l10n_ar_tribute_afip_code != '06' THEN aml.balance ELSE Null END))
            = MIN(ABS(CASE WHEN ntg.l10n_ar_vat_afip_code IS NULL AND ntg.l10n_ar_tribute_afip_code != '06' THEN aml.balance ELSE Null END))
        THEN Null
        ELSE 
            CASE WHEN sum(CASE WHEN ntg.l10n_ar_vat_afip_code IS NULL AND ntg.l10n_ar_tribute_afip_code != '06' THEN aml.balance ELSE Null END) < 0
                THEN MIN(CASE WHEN ntg.l10n_ar_vat_afip_code IS NULL AND ntg.l10n_ar_tribute_afip_code != '06' THEN aml.balance ELSE Null END)
                ELSE 
                    CASE WHEN sum(CASE WHEN ntg.l10n_ar_vat_afip_code IS NULL AND ntg.l10n_ar_tribute_afip_code != '06' THEN aml.balance ELSE Null END) > 0
                        THEN MAX(CASE WHEN ntg.l10n_ar_vat_afip_code IS NULL AND ntg.l10n_ar_tribute_afip_code != '06' THEN aml.balance ELSE Null END)
                        ELSE Null
                    END
            END
    END as icl_tax,
    CASE
        WHEN sum(ABS(CASE WHEN ntg.l10n_ar_vat_afip_code IS NULL AND ntg.l10n_ar_tribute_afip_code != '06' THEN aml.balance ELSE Null END))
            = MAX(ABS(CASE WHEN ntg.l10n_ar_vat_afip_code IS NULL AND ntg.l10n_ar_tribute_afip_code != '06' THEN aml.balance ELSE Null END))
        THEN Null
        ELSE 
            CASE WHEN sum(CASE WHEN ntg.l10n_ar_vat_afip_code IS NULL AND ntg.l10n_ar_tribute_afip_code != '06' THEN aml.balance ELSE Null END) < 0
                THEN MAX(CASE WHEN ntg.l10n_ar_vat_afip_code IS NULL AND ntg.l10n_ar_tribute_afip_code != '06' THEN aml.balance ELSE Null END)
            ...
            END
    END as idc_tax,
    sum(CASE WHEN btg.l10n_ar_vat_afip_code IN ('5', '4', '6', '9', '8', '0', '1', '2', '3', '7')THEN aml.balance ELSE Null END) as net_total,
