            ...
            if context and 'partner_dic' in context:
                ...
                for partner_obj in import_obj.padron_type_id.line_partner_ids:
                    if not partner_obj.vat:
                        raise ValidationError(_("Error: The contact {} ID({}) does not have a VAT identification number.".format(partner_obj.name, partner_obj.id)))
                    if partner_obj.l10n_latam_identification_type_id.country_id.code != 'AR':
                        raise ValidationError(_("Error: The contact {} ID({}) is not from Argentina.".format(partner_obj.name, partner_obj.id)))
