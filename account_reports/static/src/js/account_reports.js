    ...
    var accountReportsWidget = AbstractAction.extend({
        ...
        render_searchview_buttons: function() {
            ...
            if (this.report_options.product_category) {            
                ...
                var fields = {};  
                var data = {};
                var $clearProductCategoryButton = $('<button>', {
                    text: _t('Clear'),
                    class: 'clear-product-category-btn',
                    click: function() {
                        self.trigger_up('value_changed', data);
                        delete self.M2MFilters;
                    }
                });
                this.$searchview_buttons.find('.js_account_product_category_m2m').append($clearProductCategoryButton);
            }
    
            // product category filter
            if (this.report_options.product_category) {
                console.log('entro al if de product category')
                if (!this.M2MFilters) {
                    console.log(' entro al if !this.M2MFilters')
                    var fields = {};
                    if ('product_category_ids' in this.report_options) {
                        console.log('entro al if(product_category_ids in this.report_options)')
                        fields['product_category_ids'] = {
                    ...
                    if (!_.isEmpty(fields)) {
                        console.log('!_.isEmpty(fields)')
                        this.M2MFilters = new M2MFilters(this, fields);
                        this.M2MFilters.appendTo(this.$searchview_buttons.find('.js_account_product_category_m2m'));
                        console.log('this.M2MFilters.appendTo',this.M2MFilters.appendTo(this.$searchview_buttons.find('.js_account_product_category_m2m')));     
                    }
                    console.log('this.M2MFilters.$el:', this.M2MFilters.$el)
                } else {
                    console.log('entro al else')
                    this.$searchview_buttons.find('.js_account_product_category_m2m').append(this.M2MFilters.$el);
                }
                console.log('this.searchview_buttons product category',this.$searchview_buttons)
            }
